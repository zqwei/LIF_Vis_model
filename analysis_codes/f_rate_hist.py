...
for grating_id in xrange(8, 240, 30):
   for trial_id in xrange(10):
        stim = 'g%d'%grating_id
        f_name = '../simulation_ll2/output_ll2_%s_%d_sdlif_z101/tot_f_rate.dat' % (stim, trial_id)
        ...
        plt.show()
        plt.savefig('f_rate_hist/ll2_%s_%d_sdlif_z101.png' % (stim, trial_id))


for stim in ['spont']:
    for trial_id in xrange(20):
        # Load simulation data and create histograms.
        #f_name = '../simulation_ll2/output_ll2_g%d_%d_sd278/tot_f_rate.dat' % (grating_id, trial_id)
        f_name = '../simulation_ll2/output_ll2_%s_%d_sdlif_z101/tot_f_rate.dat' % (stim, trial_id)
        # print 'Processing file %s.' % (f_name)
        rates = np.genfromtxt(f_name, delimiter=' ')[:, 1] # Assume all files have the same columns of gids; use the 2nd column for rates.
