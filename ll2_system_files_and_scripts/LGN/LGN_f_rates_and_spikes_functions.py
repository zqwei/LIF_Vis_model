...
def define_cell_parameters(cells, x_start, y_start, L_x, L_y, f_out_base):
  ...
  for gid in cells:
    ...
    if (cell_type_tag == 'transient_ON'):
      scaling_factor = 0.0051 * random_factor #0.019 #0.05 #0.008
    elif (cell_type_tag == 'transient_OFF'):
      scaling_factor = 0.0051 * random_factor #0.019 #0.042 #0.00669
    elif (cell_type_tag == 'transient_ON_OFF'):
      scaling_factor = 0.003 * random_factor #0.011 #0.0294 #0.0043

    # Define the k_alpha parameter for the temporal filter.
    k_alpha_random_factor = 1.0 + 0.01 * (2.0 * random() - 1.0)
    if (cell_type_tag == 'transient_ON'):
      k_alpha = 0.038 * k_alpha_random_factor #0.038 #0.056 #0.057 #0.05 #0.062 #0.072 #0.08 #0.067 #0.168 * k_alpha_random_factor
    elif (cell_type_tag == 'transient_OFF'):
      k_alpha = 0.038 * k_alpha_random_factor #0.038 #0.057 #0.143 * k_alpha_random_factor
    elif (cell_type_tag == 'transient_ON_OFF'):
      k_alpha = 0.038 * k_alpha_random_factor #0.037 #0.055 #0.64 #0.071 #0.177 * k_alpha_random_factor
...
def uniform_f_rate(t_list, cells, f_out_base):
  ...
  f_f_rate = open(f_out_base + '_f_rate.pkl', 'w') #dat', 'w')
  pickle.dump(f_rate_dict, f_f_rate)
  f_f_rate.close()
  #return t_list, f_rate_lists
...
def mat_movie_to_f_rate(mat_movie_fname, mat_movie_objname, movie_frame_rate, cells, f_out_base, screen_size_lindeg):
  ...
  #plt.imshow(m1[0], cmap = cm.Greys_r)
  #plt.show()
  ...
  m1 = m1.astype(int)
  print 'done.'
  #for k in xrange(0, 3000, 100):
  #  plt.imshow(m1[k], cmap = cm.Greys_r)
  #  plt.show()
  #quit()
  #print Nx, Ny
  #print screen_size_lindeg

  #tmp = []
  #for i in xrange(3000):
  #  tmp.append(m1[i][96][48])
  #plt.plot(range(len(tmp)), tmp); plt.show()
  #quit()

  #tmp = []
  #for i in xrange(Nx):
  #  tmp.append(m1[0][i][int(Ny/2)])
  #plt.plot( (np.array(range(len(tmp))) * screen_size_lindeg[0] / (1.0*Nx)) , tmp, '-o'); plt.xlabel('x (lindegs)'); plt.show()
  ...
  for gid in cells:
    ...
    #tmp_rate_list = []
    #if (tmp_dict['cell_type_tag'] in ['transient_ON']):
    #  tmp_rate = np.array(f_rate[1000:1700]) # ON flash at index 1000.
    #  tmp_rate_list.append(tmp_rate)
    #elif (tmp_dict['cell_type_tag'] in ['transient_OFF']):
    #  tmp_rate = np.array(f_rate[2000:2700]) # OFF flash at index 2000.
    #  tmp_rate_list.append(tmp_rate)
    #elif (tmp_dict['cell_type_tag'] in ['transient_ON_OFF']):
    #  tmp_rate = np.array(f_rate[1000:1700]) # ON flash at index 1000.
    #  tmp_rate_list.append(tmp_rate)
    #  tmp_rate = np.array(f_rate[2000:2700]) # OFF flash at index 2000.
    #  tmp_rate_list.append(tmp_rate)
    #
    #for tmp_rate in tmp_rate_list:
    #  ind_max = tmp_rate.argmax()
    #  t_max = ind_max * 1.0 # dt = 1.0
    #  f_max = tmp_rate[ind_max]
    #  ind_decay = np.where( tmp_rate[ind_max:-1] < f_max/np.exp(1.0) )[0][0] # First index where the condition is satisfied.
    #  t_decay = t_max + ind_decay * 1.0 # dt = 1.0
    #  print 't_max, t_decay = ', t_max, t_decay
    #
    #plt.plot(range(len(f_rate)), f_rate); plt.show()
...
# Read in a sequence of images and combine them into a movie showing static images interspersed with gray screen;
...
def image_sequence_to_f_rate(image_f_list, Nx, Ny, Nt, Nt1, Nt_p, movie_frame_rate, cells, f_out_base, screen_size_lindeg):
  ...
  #im_gray[0, 0] = 0.0 # To depict this in a real scale.
  #im_gray[-1, -1] = 255.0
  #plt.imshow(im_gray, cmap=plt.cm.Greys_r); plt.show()
  ...
  for image_f_name in image_f_list:
    ...
    if (Nx_big % Nx != 0):
      ...
      Nx_add = 0
    #print im.shape
    #plt.imshow(im, cmap=plt.cm.Greys_r); plt.show()
  ...
  #print m1.shape
  #for i in xrange(0, m1.shape[0], 100):
  #  print i
  #  plt.imshow(m1[i], cmap=plt.cm.Greys_r); plt.show()
  #quit()
...
def npy_movie_to_f_rate(npy_movie_fname, original_frame_rate, Nx, Ny, Nt1, movie_frame_rate, cells, f_out_base, screen_size_lindeg):
  ...
  for i_new in xrange(new_Nm_t):
    ...
    k1 = k0 + 1 # Next frme of the original movie.
    dk = k1 - tmp_k
    # Interpolation.
    if (k1 < Nm_t):
  ...
  for i in xrange(new_Nm_t):
    ...
    #plt.imshow(im_small, cmap=plt.cm.Greys_r); plt.show()

  #print m1.shape
  #for i in xrange(0, m1.shape[0], 50):
  #  print i
  #  plt.imshow(m1[i], cmap=plt.cm.Greys_r); plt.show()
  ...
  #print m1.shape
  #for i in xrange(0, m1.shape[0], 5):
  #  print i
  #  plt.imshow(m1[i], cmap=plt.cm.Greys_r); plt.show()
...
def run_f_rate_to_spike_train(f_rate_dict, spike_trains_t_window_start, spike_trains_t_window_end, spike_trains_p_spike_max, N_trains, f_out_base):
  ...
  for cell_id in f_rate_dict['cells']:
    ...
    #if (cell_id == 0):
    #  plt.plot(t, np.array(f_rate_dict['cells'][cell_id])); plt.plot(t_include, f); plt.show()
