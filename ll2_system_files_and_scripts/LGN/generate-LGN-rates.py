...
# Define characteristics of the screen and of the region to be tiled by the LGN cells.
#scr_width = 49.0 #Width of the screen in cm.
#scr_height = 27.56 #Height of the screen in cm.
#obs_dist  = 11.0 # Distance of the "observer" (the mouse eye) mouse from the screen, in cm.
#
#screen_L_x = scr_width/obs_dist * 180.0 / np.pi #Screen width in lindegs.
#screen_L_y = scr_height/obs_dist * 180.0 / np.pi #Screen height in lindegs.
...
# This assumption is used in the code right now, but it should be
# rather easy to relax it if the code is rewritten a bit.
...
#print screen_L_x, screen_L_y, x_start, x_end, y_start, y_end

# Introduce LGN cell types and numbers of cells.
LGN_cells = {}

# Add transient ON cell type.
for gid in xrange(len(LGN_cells), len(LGN_cells) + 3000): #1900): #38
  LGN_cells.update( {gid : {'cell_type_tag': 'transient_ON'}} ) #, 'transient': True, 'light_sensitive': True}} )

# Add transient OFF cell type.
for gid in xrange(len(LGN_cells), len(LGN_cells) + 3000): #4050): #81
  LGN_cells.update( {gid : {'cell_type_tag': 'transient_OFF'}} )  #, 'transient': True, 'light_sensitive': False}} )

# Add transient ON-OFF cell type.
for gid in xrange(len(LGN_cells), len(LGN_cells) + 3000): #3050): #61
  LGN_cells.update( {gid : {'cell_type_tag': 'transient_ON_OFF'}} )  #, 'transient': True, 'light_sensitive': False}} )
...
for i in (range(6, 240, 30) + range(7, 240, 30) + range(8, 240, 30) + range(9, 240, 30) + range(10, 240, 30)):#xrange(1, 271):
...
for movie_name in ['TouchOfEvil_frames_1530_to_1680', 'TouchOfEvil_frames_3600_to_3750', 'TouchOfEvil_frames_5550_to_5700']: #['TouchOfEvil_frames_3600_to_3690', 'TouchOfEvil_frames_5550_to_5640', 'Protector2_frames_500_to_590', 'Protector2_frames_3050_to_3140', 'Protector2_frames_4700_to_4790']:
