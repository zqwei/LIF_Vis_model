...
class FilterPopulation():
    ...
    def calculate_f_rate(self, x, y, x_y_offset, cell_type_tag, sigma_c_lindegs, sigma_s_lindegs, r0, scaling_factor, k_alpha):
        ...
        if (cell_type_tag in ['transient_ON_OFF']):
          ...
          #plt.plot(range(len(l_t_vec1)), l_t_vec1); plt.plot(range(len(l_t_vec2)), l_t_vec2); plt.plot(range(len(target_r_vec)), target_r_vec);
          #plt.show()
    ...
    # This function was throughly rewritten.  The main change is that instead of taking a convolution of the subimage with the filter
    # (returning something like a blurred subimage) and summing up its elements, we take an integral (sum) of the element-wise product of
    # the subimage with the filter (the latter being centered on the center of the subimage).
        ...
        # Parameters below are those we tried to use previously with the center-surround filter.
        # The choice of these parameters seems to have been suboptimal, as the filters are super wide.
        #filter_center = 0
        #sigma_c = 8.0 * self.pixel_per_lindeg # Used to be 4.3991 pixels for 64 x 32 screen, but that seems to make a very wide filter.
        #sigma_s = 18.0 * self.pixel_per_lindeg # Used to be 9.7758 pixels for 64 x 32 screen, but that seems to make a very wide filter.
        #A_c = 0.02
        #A_s = 0.01
        #tol = 1e-10

        # One can try using a single Gaussian filter, without the surround, but we don't really have the data to argue that surround does not
        # exist for these cells.
        #filter_center = 0
        #sigma_c = 2.5 * self.pixel_per_lindeg # Consider the diameter of the filter to be ~6*sigma_c; the experimental data suggest that the diameter is ~10-12 degrees or so.
        #A_c = 1.0 # Not sure we even need this constant, as the scale is arbitrary and has to be adjusted later on anyway.
        #tol = 1e-8
        ...
        #subimage_radius = int(ceil(5.0 * sigma_c))
        subimage_radius = int(ceil(3.5 * sigma_s))
        filter_width = 2 * subimage_radius + 1

        # It looks like this filter operates with x,y coordinates in units of pixels.  This is likely not optimal, since the parameters of the filter
        # are then tied to the pixel size, rather than biologically relevant angles in the visual space.
        ...
        #plt.imshow(gaussian_filter, cmap = cm.Greys_r)
        #plt.show()

        #tmp_x = gaussian_filter[:, 26]
        #print gaussian_filter.shape, numpy.sum(gaussian_filter), numpy.sum(tmp_x)
        #plt.plot(range(tmp_x.shape[0]), tmp_x); plt.show()
        #quit()
        ...
        for frame_index in range(0, len(self.movie_frames)):
            ...
            #if (frame_index < 1000):
            #  receptive_field_subimage = 127.5 + np.zeros((x_end_subimage + 1 - x_start_subimage, y_end_subimage + 1 - y_start_subimage))
            #elif (frame_index < 2000):
            #  receptive_field_subimage = 255.0 + np.zeros((x_end_subimage + 1 - x_start_subimage, y_end_subimage + 1 - y_start_subimage))
            #else:
            #  receptive_field_subimage = 127.5 + np.zeros((x_end_subimage + 1 - x_start_subimage, y_end_subimage + 1 - y_start_subimage))
            ...
            #if ((frame_index %100 == 0) and (frame_index < 800)):
            #  plt.imshow(image_filter_product, cmap = cm.Greys_r)
            #  plt.show()
            ...
            # a semi-arbitrary fudge factor is used later on anyway, so we can as well ignore the dx*dy factor here.
        ...
        #print 'light_sensitive = ', light_sensitive
        #plt.plot(range(len(l_xy_array)), l_xy_array); plt.show()
        #quit()
