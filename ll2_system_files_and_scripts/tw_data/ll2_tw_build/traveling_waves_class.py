...
class TravelingWave():
    ...
    def generate_rates(self, **kwargs):
        ...
        #t = (random.random() - 0.5) * self.t_between_fluct
        #t = -2.0 * (self.t_up_wave + self.t_between) * (random.random() - 0.5)
...
#       for i_t in xrange(0, t_array.size, 10):
#           for src_id in src_dict:
#               x = src_dict[src_id]['x']
#               y = src_dict[src_id]['y']
#               f = f_rate_dict['cells'][src_id][i_t]
#               plt.scatter(x, y, c=f, s=200, vmin=0, vmax=1.0)
#           plt.colorbar()
#           plt.title('t = %g ms' % (t_array[i_t]))
#           plt.xlim((self.r_center[0] - Lx/2, self.r_center[0] + Lx/2))
#           plt.ylim((self.r_center[1] - Ly/2, self.r_center[1] + Ly/2))
#           plt.show()
#
#        for src_id in src_dict:
#            x = src_dict[src_id]['x']
#            y = src_dict[src_id]['y']
#            if ((abs(x) < 350.0) and (abs(y) < 350.0)):
#                f = f_rate_dict['cells'][src_id]
#                plt.plot(t_array, f); plt.title('Src %d' % (src_id)); plt.margins(0.2); plt.show()
