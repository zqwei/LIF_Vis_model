...
f_list = []
for i in xrange(0, 10):
  f_list.append('output_%s_Protector2_frames_3050_to_3140_%d/spk.dat' % (sys_name, i))
...
#for stim_name in ['spont']:
  for sd in []: #[277, 278, 279]: #xrange(277, 280):
    f_list = ['output_%s_%s_1_sd%d_test500ms/spk.dat' % (sys_name, stim_name, sd)]
    out_f_name = 'f_rate_t_by_type_%s_%s_1_sd%d_test500ms.pkl' % (sys_name, stim_name, sd)
    f_rate_t_by_type(gids_by_type, bin_start, bin_stop, bin_size, f_list, out_f_name)
...
for stim_name in ['flash_1']:
  for sd in []: #[254] + range(278, 279):
    f_name_list.append('f_rate_t_by_type_%s_%s_sd%d.pkl' % (sys_name, stim_name, sd))
...
#f_name_list.append('f_rate_t_by_type_ll1_flash_1_sd278.pkl')
