...
exp_peaks = []
exp_peaks_t = []
exp_peaks.append(np.load('/allen/aibs/mat/antona/experimental_data/2017_07_25_ephys_Sev_processed_by_Yazan/flashes/average_peaks_1.npy'))
exp_peaks.append(np.load('/allen/aibs/mat/antona/experimental_data/2017_07_25_ephys_Sev_processed_by_Yazan/flashes/average_peaks_2.npy'))
exp_peaks_t.append(np.load('/allen/aibs/mat/antona/experimental_data/2017_07_25_ephys_Sev_processed_by_Yazan/flashes/average_time_to_peak_1.npy'))
exp_peaks_t.append(np.load('/allen/aibs/mat/antona/experimental_data/2017_07_25_ephys_Sev_processed_by_Yazan/flashes/average_time_to_peak_2.npy'))
...
for i_peak in xrange(len(peaks)):
    ax[1].scatter(i_peak*np.ones(peaks[i_peak].size), peaks[i_peak], s=150, lw=0, facecolor='firebrick')
    ind = np.where(peaks[i_peak] > y_lim_top[0])[0]
    ax[1].annotate(u'$\u2191$'+'\n%d/%d' % (ind.size, peaks[i_peak].size), xy=(i_peak, 1.0*y_lim_top[0]), fontsize=12)

    ax[1].scatter(i_peak*np.ones(exp_peaks[i_peak].size)+exp_displ, exp_peaks[i_peak], s=150, lw=0, facecolor='gray')
    ...
    ax[2].scatter(i_peak*np.ones(peaks_t[i_peak].size), peaks_t[i_peak], s=150, lw=0, facecolor='firebrick')
    ind = np.where(peaks_t[i_peak] > y_lim_top[1])[0]
    ax[2].annotate(u'$\u2191$'+'\n%d/%d' % (ind.size, peaks_t[i_peak].size), xy=(i_peak, 1.0*y_lim_top[1]), fontsize=12)

    ax[2].scatter(i_peak*np.ones(exp_peaks_t[i_peak].size)+exp_displ, exp_peaks_t[i_peak], s=150, lw=0, facecolor='gray')
...
plt.savefig('flashes/flashes_frt_ll2_ll3_flash_2.eps', format='eps')
plt.show()
