...
Ncores = Nnodes * ppn
tstop = 500
f_name = 'rl2_g8_8_test%dms_inh_lif_syn_z' % (tstop)
...
def syn_weight_amp(output_folder, syn_file=''):
    ...
    with open('syn_data_lif_z101.jsonbak') as data_file:
        ...
        data["Scnn1a"]["LGN_exc"]["w"] = amp_syn[0] * data["Scnn1a"]["LGN_exc"]["w"]
        data["Rorb"]["LGN_exc"]["w"] = amp_syn[1] * data["Rorb"]["LGN_exc"]["w"]
        data["Nr5a1"]["LGN_exc"]["w"] = amp_syn[2] * data["Nr5a1"]["LGN_exc"]["w"]
        data["PV1"]["LGN_exc"]["w"] = amp_syn[3] * data["PV1"]["LGN_exc"]["w"]
        data["PV2"]["LGN_exc"]["w"] = amp_syn[4] * data["PV2"]["LGN_exc"]["w"]
        with open('syn_data_lif_z' + syn_file + '.json', 'w') as outfile:  # rewrite the syn file
            json.dump(data, outfile, indent=4)
    with open('config_rl2_g8_8_sd285_LGN_only_no_con_LGN_tp_PV_x2.json') as data_file:  # rewrite the config file
        data = json.load(data_file)
