...
def syn_weight_amp(output_folder, syn_file=''):
    with open('syn_data_lif_z101.jsonbak') as data_file:
        ...
        data["Scnn1a"]["LGN_exc"]["w"] = 1.0 * data["Scnn1a"]["LGN_exc"]["w"]
        data["Rorb"]["LGN_exc"]["w"] = 1.0 * data["Rorb"]["LGN_exc"]["w"]
        data["Nr5a1"]["LGN_exc"]["w"] = 1.0 * data["Nr5a1"]["LGN_exc"]["w"]
        data["PV1"]["LGN_exc"]["w"] = 2.0 * data["PV1"]["LGN_exc"]["w"]
        data["PV2"]["LGN_exc"]["w"] = 2.0 * data["PV2"]["LGN_exc"]["w"]
        with open('syn_data_lif_z' + syn_file + '.json', 'w') as outfile:  # rewrite the syn file
            json.dump(data, outfile, indent=4)
    with open('config_rl2_g8_8_sd285_LGN_only_no_con_LGN_to_PV_x2.json') as data_file:  # rewrite the config file
        data = json.load(data_file)
