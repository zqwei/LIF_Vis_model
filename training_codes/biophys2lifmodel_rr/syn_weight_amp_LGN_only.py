...
Ncores = Nnodes * ppn
tstop = 500
f_name = 'rr2_g8_8_test%dms_inh_lif_syn_z' % (tstop)
...
def syn_weight_amp(output_folder, syn_file=''):
    with open('syn_data_lif_z101.jsonbak') as data_file:
        ...
        data["PV1"]["LGN_exc"]["w"] = 1.0 * data["PV1"]["LGN_exc"]["w"]
        data["PV2"]["LGN_exc"]["w"] = 1.0 * data["PV2"]["LGN_exc"]["w"]
        with open('syn_data_lif_z' + syn_file + '.json', 'w') as outfile:  # rewrite the syn file
            json.dump(data, outfile, indent=4)
    with open('config_rr2_g8_8_sd282_cn0_LGN_only_no_con.json') as data_file:  # rewrite the config file
        data = json.load(data_file)
