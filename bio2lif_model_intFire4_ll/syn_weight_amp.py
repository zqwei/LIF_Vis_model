...
def syn_weight_amp(output_folder, syn_file=''):
    ...
    with open('syn_data_lif_z101.jsonbak') as data_file:
        ...
        data["Scnn1a"]["exc"]["w"] = amp_syn[0] * data["Scnn1a"]["exc"]["w"]
        data["Rorb"]["exc"]["w"] = amp_syn[1] * data["Rorb"]["exc"]["w"]
        data["Nr5a1"]["exc"]["w"] = amp_syn[2] * data["Nr5a1"]["exc"]["w"]
        data["PV1"]["exc"]["w"] = amp_syn[3] * data["PV1"]["exc"]["w"]
        data["PV2"]["exc"]["w"] = amp_syn[4] * data["PV2"]["exc"]["w"]
        data["Scnn1a"]["inh"]["w"] = amp_syn[5] * data["Scnn1a"]["inh"]["w"]
        data["Rorb"]["inh"]["w"] = amp_syn[6] * data["Rorb"]["inh"]["w"]
        data["Nr5a1"]["inh"]["w"] = amp_syn[7] * data["Nr5a1"]["inh"]["w"]
        data["PV1"]["inh"]["w"] = amp_syn[8] * data["PV1"]["inh"]["w"]
        data["PV2"]["inh"]["w"] = amp_syn[9] * data["PV2"]["inh"]["w"]
        with open('syn_data_lif_z' + syn_file + '.json', 'w') as outfile:  # rewrite the syn file
            json.dump(data, outfile, indent=4)
    with open('config_ll2_g8_8_sdlif_z101_test500ms.json') as data_file:  # rewrite the config file
        data = json.load(data_file)
