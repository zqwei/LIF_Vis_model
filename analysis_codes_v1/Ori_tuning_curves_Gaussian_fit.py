...
sys_dict['ll2'] = {'cells_file': cell_db_path+'ll2.csv',
                    ...
                    'f_out_Gfit': 'Ori/ll2_pref_stat_and_Gfit.csv',
...
sys_dict['ll2_TF4Hz'] = { 'cells_file': cell_db_path+'ll2.csv',
                    ...
                    'f_out_Gfit': 'Ori/ll2_pref_stat_and_Gfit_4Hz.csv',
...
for sys_name in sys_dict.keys():
    ...
    for k_cell, gid in enumerate(sim_data[sys_name]['id'].values):
        ...
        # Treat ori_pref as a constant.
        #ori_pref = ori_float_sorted[av.argmax()] # Orientation corresponding to the maximum of av.
        #params_start = [5.0, 5.0, 0.1, 50.0]
        #fit_params, cov, infodict, mesg, ier = leastsq( double_gaussian_fit, params_start, args=(ori_float_sorted, ori_pref, av), full_output=True )
        ...
        #plt.errorbar(ori_float_sorted, av, yerr=std, marker='o', ecolor='k')
        #ori_tmp = np.arange(0.0, 360.0, 1.0)
        ## Treat ori_pref as a constant.
        ##plt.plot(ori_tmp, double_gaussian(ori_tmp, ori_pref, fit_params), c='r')
        ## Treat ori_pref as one of the parameters.
        #plt.plot(ori_tmp, double_gaussian(ori_tmp, fit_params), c='r')
        #plt.xticks(ori_float_sorted)
        #plt.ylim(bottom=0.0)
        #plt.xlim((0.0, 360.0))
        #plt.title('Gid = %d, rsquared = %f' % (gid, rsquared))
        #plt.show()
