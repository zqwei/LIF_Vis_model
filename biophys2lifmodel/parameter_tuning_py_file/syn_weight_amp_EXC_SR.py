...
def syn_weight_amp(output_folder, syn_file=''):
    ...
    amp_syn_S = all_data['w_curr'][0]
    amp_syn_R = all_data['w_curr'][1]
    with open('syn_data_278_lif_amp_100_EXC_Nra.jsonbak') as data_file:
        data = json.load(data_file)
        data["Scnn1a"]["exc"]["w"] = amp_syn_S * data["Scnn1a"]["exc"]["w"]
        data["Rorb"]["exc"]["w"] = amp_syn_R * data["Rorb"]["exc"]["w"]
