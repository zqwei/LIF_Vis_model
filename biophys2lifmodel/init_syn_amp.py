...
def estimate_initial_error(amp_syn_output, syn_file, ncol=2, ref_file=ref_file):
    ...
    for cell_group in xrange(len(cell_type)):
        ...
        # 'w_old' from initialized file
        pdUpdateNew['w_old'][cell_group] = amp_syn[cell_group, 2]
...
def main(starting_idx):
    if not exists('output_'+f_name+str(starting_idx)+'/tot_f_rate.dat'):
        syn_weight_amp('syn_amp_trial_112', str(starting_idx))
    else:
        estimate_initial_error('syn_amp_trial_112', str(starting_idx))
        plot_tot_firing_rate_comparison('output_'+f_name+str(starting_idx))
