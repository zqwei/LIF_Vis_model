...
def external_inputs(ext_in_tar_cell_gid, ext_inp_path, tar_ext_inp_map, ext_inp_dict, t_stop, utils_obj):
  ...
  if (mode == 'file'):
    ...
    f_train_line_pos = tar_ext_inp_map['src_gid'].values * N_trials_in_file + ext_inp_trial + 1
  elif (mode == 'random'):
  ...
  for i_line in xrange(N_syn.size):
    ...
    for j in xrange(N_syn[i_line]):
      if (mode == 'random'):
        ...
        dumNetStim.interval = random_ISI[i_line]
        dumNetStim.number = 5 + int(t_stop / random_ISI[i_line])
